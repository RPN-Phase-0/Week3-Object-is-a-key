...
function digitPerkalianMinimum(angka) {
  ...
  let terkecil = (
    hasil[hasil.length / 2].toString() + hasil[hasil.length / 2 - 1].toString()
  ).length;
  for (let j = 0; j < hasil.length; j++) {
    let ter = (hasil[j].toString() + hasil[hasil.length - 1 - j].toString())
      .length;
    if (ter < terkecil) {
      terkecil = ter;
    }
...
console.log(digitPerkalianMinimum(100)); // 3
