...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < productList.length; i++) {
    const productName = productList[i][0];
    const productPrice = productList[i][1];
    const productQty = productList[i][2];
    ...
    for (let j = 0; j < shoppers.length; j++) {
      ...
      if (customer.product === productName && data.leftOver >= customer.amount) {
        data.shoppers.push(customer.name);
        data.leftOver -= customer.amount;
        data.totalProfit += productPrice * customer.amount;
      }
    }
