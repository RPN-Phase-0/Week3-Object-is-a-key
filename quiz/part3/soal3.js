...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < shoppers.length; i++){
        let pembeli = shoppers[i];
        let nama = pembeli.name;
        let item = pembeli.product;
        let jumlah = pembeli.amount;
        if(item == Sepatu.product){
            if(Sepatu.letfOver >= jumlah){
                Sepatu.shoppers.push(nama);
                Sepatu.letfOver -= jumlah;
                Sepatu.totalProfit += jumlah*listBarang[0][1];
            }
        }else if(item == Baju.product){
            if(Baju.letfOver >= jumlah){
                Baju.shoppers.push(nama);
                Baju.letfOver -= jumlah;
                Baju.totalProfit += jumlah*listBarang[1][1];
            }
        }else if(item == Sweater.product){
            if(Sweater.letfOver >= jumlah){
                Sweater.shoppers.push(nama);
                Sweater.letfOver -= jumlah;
                Sweater.totalProfit += jumlah*listBarang[2][1];
            }
        }
    }
    result.push(Sepatu);
