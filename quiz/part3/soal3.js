/*
Toko X yang sedang melakukan SALE ingin menghitung jumlah profit untuk setiap jenis barang yang 
...
dan jumlah barang yang dibelinya). Jika stock barang kurang dari jumlah yang ingin dibeli oleh 
pembeli maka pembeli batal untuk membeli barang tersebut.
...
function countProfit(shoppers) {
    ...
    for(let cust of shoppers) {
        for(let i = 0; i < result.length; i++) {
            let item = result[i];
            if(cust.product === item.product && cust.amount <= item.leftOver) {
                item.shoppers.push(cust.name);
                item.leftOver -= cust.amount;
                item.totalProfit += cust.amount * harga[i];
            }
        }
    }
    return result;
