...
function shoppingTime(memberId, money) {
  const items = [
    {
      item: "Sepatu brand Stacattu",
      price: 1500000,
    },
    { item: "Baju brand Zoro", price: 500000 },
    { item: "Baju brand H&N", price: 250000 },
    { item: "Sweater brand Uniklook", price: 175000 },
  ...
  for (let i = 0; i < items.length; i++) {
    if (result.changeMoney >= items[i].price) {
      result.changeMoney = result.changeMoney - items[i].price;
      result.ListPurchased.push(items[i].item);
    }
  }
  return result;
}
