...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    const nameProduct = listBarang[i][0];
    const price = listBarang[i][1];
    const stockRemaining = listBarang[i][2];

    const detail = {
      product: nameProduct,
      shoppers: [],
      leftOver: stockRemaining,
      totalProfit: 0,
    };
    ...
    for (let j = 0; j < shoppers.length; j++) {
      ...
      if (
        customer.product === nameProduct &&
        detail.leftOver >= customer.amount
      ) {
        detail.shoppers.push(customer.name);
        detail.leftOver -= customer.amount;
        detail.totalProfit += price * customer.amount;
      }
    }
